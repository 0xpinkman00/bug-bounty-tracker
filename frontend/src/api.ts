...
export type ReportSection = {title:string; body:string; placement:'before'|'after'};
export type VulnerabilityReport = {id:number; title:string; severity:string; report_type:string; target:string|null; impacts:string[]; brief:string; vulnerability_details:string; impact_details:string; references:string; proof_of_concept:string; recommendation:string; details:{label:string;value:string}[]; extra_sections:ReportSection[]; tags:string[]; fix_url:string|null; source_url:string|null; reported_at:string|null; program_id:number|null; repository_id:number|null; program_name:string|null; repository_name:string|null; created_at:string; updated_at:string};
...
export type Program = {id:number; name:string; platform:string; platform_program_id:string; program_url:string; max_bounty:string|null; status:string; first_seen_at:string; last_changed_at:string|null; latest_activity_at?:string|null; latest_update?:ProgramUpdate|null; updates?:ProgramUpdate[]; impacts?:Impact[]; assets?:Asset[]; repositories?:Repository[]; events?:Event[]};
export type Asset = {id:number; type:string; value:string; url:string|null; in_scope:boolean; repository_id?:number|null; latest_release?:Release|null};
...
export type ProgramScanRun = {id:string; status:'queued'|'running'|'completed'|'completed_with_errors'|'rate_limited'|'failed'; rate_limit_reset_at?:string; started_at:string; completed_at:string|null; program_updated:boolean|null; total_repositories:number; checked_count:number; new_release_count:number; errors:string[]};
